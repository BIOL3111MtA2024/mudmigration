...
```{r}
top_data <- filter(chl_data, Top_Bottom == 'top', Chl_ugcm2 > 0, YYYYMMDDHHMM == 202410101430)
top_data$Light_Temp <- factor(paste(top_data$Temp_C, top_data$Light))
...
top_data %>%
  ggplot() +
  geom_point(aes(x = as.factor(Light_Temp), y = Chl_ugcm2, colour = 'pink')) +
  theme_bw() +
