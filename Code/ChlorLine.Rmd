...
No significant trend in all the three temperature settings, based on statistical test ran and wide confidence interval visualized on the plot.

There is migration happening, however, not at a measurable trend with temperature, since the data has very few points and is very sparse.

In all temperature settings, values started low, and increased later on - but still, high values were not found at the start of the sampling period.
As time went on, the range of values increased.
...
unnest(cols = c(LinearTidy)) |>
 select(-c(data, LinearFit, LinearParam, LinearPredict)) |>
  ...
  kable()
```
From the results of the nested regression, there is not a significant trend in any of the temperature settings.
...
summary(aov(TopBottomRatio ~ ToD * Temp_C,
           data = vert_migration_wide))
```
Both the linear model and the ANOVA suggest that neither ToD nor the interaction between ToD and Temperature significantly affect the TopBottomRatio variable. There is a borderline significance for temperature in the ANOVA test, hinting at a possible, but minor effect on the relationship.

The R-squared values in the linear model indicate that the predictors explain very little of the variability in the TopBottomRatio variable.


We have high top to bottom ratios, but a simple linear regression does not show that
