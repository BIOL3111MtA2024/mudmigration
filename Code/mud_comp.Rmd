...
Manual cell counting was done by imaging lenses using a florescent scope. Two images per lens were taken and assigned a sample photo ID. Each image were then analyzed using DFM measure. Each observed cell was categorized by morphotype and measured to give a width (two widths for pennate) and a length in um. 
...
```
### Observations:
Cell are overwhelmingly smaller or equal to 50um. Comparing top and bottom samples, there is a bigger difference in cell counts in 15 degrees.  
...
```
### Observations:
Many differences in community makeup exist between conditions. I expect that trends displayed on the figure are due to a small sample size. However, there may be an effect of temperature. 
