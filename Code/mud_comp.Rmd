...
```
#### This conversion works because, chains all centric (dimension)
...
```
#### Take the above and facet to replace other with bins

## Bin max dimension and cell volume
```{r bin_dimension}
#Maximum Cell Dimension 
bin_ranges_1 <- c(0, 5, 10, 20, 30, 40, 50, 75, 100, 150, 200)
dimension_data <- volume_dimension_data |>
  dplyr::mutate(MaxDim_Bin = cut(MaxDim_Total, breaks = bin_ranges_1, include.lowest = TRUE, labels = c("0", "5", "10", "20", "30", "40", "50", "75", "100", "150")))
```

## Merge common value between raw data and conditions (light, temp, layer)
```{r merge_sheets}
merged_data <- merge(community_composition, dimension_data, by = "SamplePhotoID", all.x = TRUE)
```
...
filtered_data <- merged_data |>
  ...
  dplyr::group_by(MaxDim_Bin, Light, Temp_C, Top_Bottom, CellMorphotype, Patch, ChainNum) |>
  dplyr::summarize(MaxDim_Count = dplyr::n(), .groups = "drop")

print(filtered_data)
```
#### Add units to dimension and volumn

```{r ratio__}
# Ratio of Top and Bottom counts within each Patch
top_bottom_ratios <- filtered_data |>
...
ggplot(top_bottom_ratios, aes(x = Patch, y = Ratio, fill = Light)) +
