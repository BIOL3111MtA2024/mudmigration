...
filtered_data |>
  filter(Light != "med") |>
  ggplot() +
  geom_histogram(aes(x = MaxDim_Total, fill = Light), breaks = c(0, 50, 150), color = "black", position = "stack") +
  labs(
    x = "Maximum Dimension (um)", 
    y = "Cell Count per 0.32 mm²", 
    title = "Total Cell Count versus Maximum Dimension, faceted by Temperature and Layer, light level as indicated by colour"
  ) +
...
# IN PROGRESS 
```{r}
ggplot(top_bottom_ratio_data, aes(x = MorphoSize, y = Ratio)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    x = "Cell Morphotype", 
    y = "Top-to-Bottom Ratio of Cell Counts per 0.32 mm2",
