...
## Plots of all morphotypes per condition 
```{r morphotype_conditions}
ggplot(volume_morphotype_data, aes(x = MaxVol_Bin, y = MaxVol_Count, fill = CellMorphotype)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    x = "Cell Volume Bins (um³)", 
    y = "Cell Count per 0.32 mm2",
    title = "Cell Volume Bins vs Morphotypes in Different Light Conditions"
  ) +
...
ggplot(dimension_morphotype_data, aes(x = MaxDim_Bin, y = MaxDim_Count, fill = CellMorphotype)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    x = "Cell Maximum Dimension Bins (um)", 
    y = "Cell Count per 0.32 mm2",
    title = "Cell Maximum Dimension Bins vs Morphotypes in Different Light Conditions"
  ) +
  facet_wrap(~ Light, ncol = 2) +
  theme_minimal() +
  theme(
...
```{r anova}
#ANOVA of volume
anova_vol_results <- aov(MaxVol_Count ~ Light * Top_Bottom * Temp_C * CellMorphotype, data = volume_morphotype_data)

summary(anova_vol_results)

#ANOVA of maximum dimension
anova_dim_results <- aov(MaxDim_Count ~ Light * Top_Bottom * Temp_C * CellMorphotype, data = dimension_morphotype_data)

summary(anova_dim_results)
```


## Testing for the effect of layers 
```{r top_bottom_analysis}
#Volume
vol_with_top_bottom <- lm(MaxVol_Count ~ MaxVol_Bin + Light + Temp_C + Top_Bottom + CellMorphotype, 
                 data = volume_morphotype_data)
vol_without_top_bottom <- lm(MaxVol_Count ~ MaxVol_Bin + Light + Temp_C + CellMorphotype, 
                    data = volume_morphotype_data)
anova(vol_without_top_bottom, vol_with_top_bottom)

#Maximum dimension
dim_with_top_bottom <- lm(MaxDim_Count ~ MaxDim_Bin + Light + Temp_C + Top_Bottom + CellMorphotype, 
                 data = dimension_morphotype_data)
dim_without_top_bottom <- lm(MaxDim_Count ~ MaxDim_Bin + Light + Temp_C + CellMorphotype, 
                    data = dimension_morphotype_data)
anova(dim_without_top_bottom, dim_with_top_bottom)
```

## Linear model
```{r linear}
#Linear model for volume 
lm_vol_results <- lm(MaxVol_Count ~ MaxVol_Bin + Light + Temp_C + Top_Bottom + CellMorphotype, data = volume_morphotype_data)

summary(lm_vol_results)

#Linear model for maximum dimension
lm_dim_results <- lm(MaxDim_Count ~ MaxDim_Bin + Light + Temp_C + Top_Bottom + CellMorphotype, data = dimension_morphotype_data)

summary(lm_dim_results)
```
