...
#Issue converting HHMM to ToD b/c leading 0 on 0900 messes up conversions & we do not have SS data to use simple lubridate::hms() function
vert_migration_2 <- vert_migration |>
  mutate(DateTime = lubridate::ymd_hm(YYYYMMDDHHMM)) |>
  
  mutate(HHMM = lubridate::hms(str_split(DateTime, pattern = " ")))
